...
namespace testing {
    ...
    TEST(TwoUtilsTest, checkFindMaxIsFiveDotZero) {
        ...
        EXPECT_EQ(LibTwoUtils::find_max(samples), 5.0) << "Check samples's max is 5.0 " ;
    }
    ...
    TEST(TwoUtilsTest, checkMeanIsOnePointTwentySix) {
        ...
        EXPECT_EQ(LibTwoUtils::sample_mean(samples), 1.26) << "Check samples's mean is 1.26 " ;
    }
    ...
    TEST(TwoUtilsTest, checkOneSampleVector) {
        ...
        ASSERT_LE(LibTwoUtils::sample_mean(samples),
                  LibTwoUtils::find_max(samples)) << "Check sample_mean and find_max manage one item's vector " ;
    }
